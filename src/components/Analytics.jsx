...
const Analytics = () => {
  ...
  const exportToCSV = (data, filename) => {
    ...
    if (filename === "visit-durations.csv") {
      ...
      Object.entries(data).forEach(([id, events]) => {
        csvContent += `${id},${events.length},"${events
          .map((d) => d.toFixed(2))
          .join(", ")}"\n`;
      });
    }
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      try {
        const [visitsResponse, durationsResponse, eventsResponse] =
          await Promise.all([
            fetch(`${ANALYTICS_URL}/currentvisits`),
            fetch(`${ANALYTICS_URL}/visitduration`),
            fetch(`${ANALYTICS_URL}/completedevents`),
          ]);
        ...
        if (!visitsResponse.ok) {
          ...
          console.log("Current visits data:", visitsData);
        }

        if (!durationsResponse.ok) {
          console.error(
            "Durations response error:",
            await durationsResponse.text()
          );
        } else {
          const durationsData = await durationsResponse.json();
          setVisitDurations(durationsData);
          console.log("Visit durations data:", durationsData);
        }

        if (!eventsResponse.ok) {
          console.error("Events response error:", await eventsResponse.text());
        } else {
    ...
    fetchAnalyticsData();
    const interval = setInterval(fetchAnalyticsData, 2000);
          ...
          <div className="relative p-0.5 rounded-2xl bg-conic-gradient">
            <div className="relative bg-n-8 rounded-[1rem] p-8 h-full">
              ...
              <p className="body-2 text-n-4">
                Currently being tracked in real-time
              </p>
            </div>
          </div>
            ...
            <div className="relative bg-n-8 rounded-[1rem] p-8 h-full">
              <div className="flex justify-between items-center mb-4">
                ...
                <Button
                  ...
                  onClick={() =>
                    exportToCSV(visitDurations, "visit-durations.csv")
                  }
                >
                  Export CSV
                </Button>
              ...
              <div className="space-y-4">
                {Object.entries(visitDurations).map(([id, duration]) => (
                  <div key={id} className="flex justify-between items-center">
                    ...
                    <span className="body-2 text-primary-1">
            ...
            <div className="relative bg-n-8 rounded-[1rem] p-8 h-full">
              <div className="flex justify-between items-center mb-4">
                ...
                <Button
                  className="text-sm px-4 py-2"
                  onClick={() =>
                    exportToCSV(completedEvents, "completed-events.csv")
                  }
                >
                  Export CSV
                </Button>
